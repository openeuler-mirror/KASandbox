...
/*	
	...
	if registrar, ok := base.(storage.BufferRegistrar); ok {
		// Mooncake 后端：4K 对齐 + buffer 注册
		const cacheAlign = int64(4096)
		alignedSize := ((size + cacheAlign - 1) / cacheAlign) * cacheAlign
		zap.L().Sugar().Infof("[FullFetchChunker] origin cache size %d bytes, block size %d bytes, aligned size %d bytes", size, blockSize, alignedSize)
		cache, err = NewCache(alignedSize, blockSize, cachePath, false)
		if err != nil {
			zap.L().Sugar().Errorf("[FullFetchChunker] Create file cache FAILED: err=%v", err)
			return nil, fmt.Errorf("failed to create file cache: %w", err)
		}

		ptr := cache.Ptr()
		if ptr != 0 {
			zap.L().Sugar().Infof("[FullFetchChunker] Begin Register cache buffer")
			sz, err := cache.Size()
			if err != nil {
				cache.Close()
				zap.L().Sugar().Errorf("[FullFetchChunker] Get cache size FAILED: err=%v", err)
				return nil, fmt.Errorf("failed to get cache size: %w", err)
			}
			if err := registrar.RegisterBuffer(ptr, uint64(sz)); err != nil {
				zap.L().Sugar().Errorf("[FullFetchChunker] Register cache buffer FAILED: ptr=%x, size=%d, err=%v", ptr, sz, err)
				cache.Close()
				return nil, fmt.Errorf("failed to register cache buffer: %w", err)
			}
			zap.L().Sugar().Infof("[FullFetchChunker] Register cache buffer OK: ptr=%x, size=%d bytes", ptr, sz)
		}
	} else {
		// 其他后端（S3/GCP/LocalFS/MinIO）：原始流程
...
// It uses an interface-assertion based dispatch: if the base storage backend implements
// storage.BatchReader (e.g. Mooncake), it performs a single batched read; otherwise it falls
// back to launching one goroutine per chunk for parallel reads (works with S3/GCP/LocalFS/etc.).
func (c *FullFetchChunker) fetchToCache(ctx context.Context, off, length int64) error {
	...
	// Try the Mooncake fast-path: a single batched backend call.
	/*if batchReader, ok := c.base.(storage.BatchReader); ok {
		return c.fetchToCacheBatch(ctx, startingChunkOffset, chunks, batchReader)
	}*/

	// Slow path: one goroutine per chunk (works for any SeekableReader backend).
...
// fetchToCacheBatch implements the Mooncake-optimised single-call read path.
// It collects memory pointers for all chunks, calls BatchReadInto once, and
// marks all chunks as cached.
/*
func (c *FullFetchChunker) fetchToCacheBatch(
	ctx context.Context,
	startingChunkOffset int64,
	chunks []int64,
	...
	// Phase 1: collect memory pointers for every chunk.
	offsets := make([]int64, n)
	sizes := make([]int64, n)
	ptrs := make([]uintptr, n)
	releaseFuncs := make([]func(), 0, n)
	fetchOffsets := make([]int64, n)

	for i, chunkOff := range chunks {
		fetchOff := startingChunkOffset + chunkOff
		fetchOffsets[i] = fetchOff

		b, release, err := c.cache.addressBytes(fetchOff, storage.MemoryChunkSize)
		if err != nil {
			// Unwind any locks we already acquired
			for _, r := range releaseFuncs {
				r()
			}
			return fmt.Errorf("failed to get cache buffer for chunk %d: %w", fetchOff, err)
		}
	...
	// Phase 2: single backend call to read all chunks.
	fetchSW := c.metrics.RemoteReadsTimerFactory.Begin()

	readBytes, err := batchReader.BatchReadInto(ctx, offsets, sizes, ptrs)

	// Phase 3: always release the read locks (even on error).
	for _, r := range releaseFuncs {
		r()
	}

	if err != nil {
		fetchSW.Failure(ctx, int64(sumInts(readBytes)),
			attribute.String(failureReason, failureTypeRemoteRead),
		)
		return fmt.Errorf("failed to batch read %d chunks from base: %w", n, err)
	}

	// Phase 4: mark each chunk as cached.
	for i, nbytes := range readBytes {
		if int64(nbytes) != sizes[i] {
			fetchSW.Failure(ctx, int64(nbytes),
				attribute.String(failureReason, failureTypeRemoteRead),
			)
			return fmt.Errorf("failed to batch read chunk %d: expected %d bytes, got %d bytes",
				fetchOffsets[i], sizes[i], nbytes)
		}
		c.cache.setIsCached(fetchOffsets[i], int64(nbytes))
	}

	fetchSW.Success(ctx, int64(sumInts(readBytes)))
	return nil
}
*/
// sumInts returns the sum of all integers in the slice.
...
func (c *FullFetchChunker) Close() error {
	// 先检查 cache 是否有效，再获取 ptr
	if c.cache != nil {
		// if registrar, ok := c.base.(storage.BufferRegistrar); ok {
		// 	ptr := c.cache.Ptr()
		// 	if ptr != 0 {
		// 		if err := registrar.UnregisterBuffer(ptr); err != nil {
		// 			zap.L().Sugar().Errorf("[FullFetchChunker] Unregister cache buffer FAILED: ptr=%x, err=%v", ptr, err)
		// 		} else {
		// 			zap.L().Sugar().Infof("[FullFetchChunker] Unregister cache buffer OK: ptr=%x", ptr)
		// 		}
		// 	}
		// }
