...
func (t *storageTemplate) Fetch(ctx context.Context, buildStore *build.DiffStore) {
	...
	fetchStart := time.Now()
	defer func() {
		zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch total cost: %.3f ms, buildID=%s",
			time.Since(fetchStart).Seconds()*1000, t.files.BuildID)
	}()
	...
	wg.Go(func() error {
		if t.localSnapfile != nil {
			...
			zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch set local snapfile cost: %.3f ms, buildID=%s",
				time.Since(t1).Seconds()*1000, t.files.BuildID)
		...
		zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch newStorageFile(snapfile) cost: %.3f ms, buildID=%s, cache=%s",
			time.Since(tSnap).Seconds()*1000, t.files.BuildID, t.files.CacheSnapfilePath())
	...
	wg.Go(func() error {
		if t.localMetafile != nil {
			...
			zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch set local metafile cost: %.3f ms, buildID=%s",
				time.Since(t1).Seconds()*1000, t.files.BuildID)
		...
		if err != nil {
			...
			zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch write v1 metafile cost: %.3f ms, buildID=%s",
				time.Since(t1).Seconds()*1000, t.files.BuildID)
		...
		zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch newStorageFile(metafile) cost: %.3f ms, buildID=%s",
			time.Since(tMeta).Seconds()*1000, t.files.BuildID)
	...
	wg.Go(func() error {
		...
		zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch NewStorage(memfile) cost: %.3f ms, buildID=%s",
			time.Since(tMem).Seconds()*1000, t.files.BuildID)
	...
	wg.Go(func() error {
		...
		zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch NewStorage(rootfs) cost: %.3f ms, buildID=%s",
			time.Since(tRootfs).Seconds()*1000, t.files.BuildID)
	...
	zap.L().Sugar().Infof("[ResumeSandbox] storageTemplate.Fetch wg.Wait (all 4 goroutines) total cost: %.3f ms, buildID=%s",
		time.Since(tWait).Seconds()*1000, t.files.BuildID)
}
