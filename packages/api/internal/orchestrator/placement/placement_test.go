...
func TestPlaceSandbox_SuccessfulPlacement(t *testing.T) {
	...
	resultNode, _, err := PlaceSandbox(ctx, algorithm, nodes, nil, sbxRequest, machineinfo.MachineInfo{})
...
func TestPlaceSandbox_WithPreferredNode(t *testing.T) {
	...
	resultNode, _, err = PlaceSandbox(ctx, algorithm, nodes, node2, sbxRequest, machineinfo.MachineInfo{})
	require.NoError(t, err)
	assert.NotNil(t, resultNode)
	assert.Equal(t, node2, resultNode)
...
func TestPlaceSandbox_NoNodes(t *testing.T) {
	...
	resultNode, _, err := PlaceSandbox(ctx, algorithm, []*nodemanager.Node{}, nil, sbxRequest, machineinfo.MachineInfo{})
