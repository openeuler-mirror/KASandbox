...
readonly DOWNLOAD_PACKAGE_PATH="/opt/e2b-infra/dep"
...
function fetch_binary {
  ...
  local -r consul_dest_path=$DOWNLOAD_PACKAGE_PATH/consul_${version}_linux_${arch}.zip
  # 首先检查目标文件是否已存在，如果存在则直接返回，跳过下载
  if [[ -f "$consul_dest_path" ]]; then
    echo "File $consul_dest_path already exists, skipping download."
    return 0
  fi
  ...
  retry \
    "curl -o '$consul_dest_path' '$download_url' --location --silent --fail --show-error" \
    "Downloading Consul to $consul_dest_path"
}
...
function install_binary {
  ...
  local -r download_file=$DOWNLOAD_PACKAGE_PATH/consul_${version}_linux_${arch}.zip
