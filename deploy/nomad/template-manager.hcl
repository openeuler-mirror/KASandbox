  ...
  group "template-manager" {
    ...
    task "start" {
      ...
      env {
        ...
        # 层文件（COPY）上传端点：CLI 直连本节点，地址需对 CLI 可达
        MOONCAKE_UPLOAD_PUBLIC_ENDPOINT = "${MOONCAKE_UPLOAD_PUBLIC_ENDPOINT}"
        MOONCAKE_UPLOAD_SIGNING_SECRET  = "${MOONCAKE_UPLOAD_SIGNING_SECRET}"
        MOONCAKE_UPLOAD_MAX_BYTES       = "${MOONCAKE_UPLOAD_MAX_BYTES}"
     }
