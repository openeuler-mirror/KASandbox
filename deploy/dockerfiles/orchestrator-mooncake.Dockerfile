...
umdk-urma-tool-${umdk_version}.${oe_arch} umdk-urma-example-${umdk_version}.${oe_arch}
...
# 检查 mooncake 和 ubdiag 包是否已安装
RUN for pkg in mooncake ubdiag; do \
        rpm -q "$pkg" >/dev/null 2>&1 || { echo "ERROR: $pkg package not installed"; exit 1; }; \
    done
