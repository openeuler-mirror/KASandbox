...
#
# 验证目标：
#   1. kubectl exec 能成功在容器中执行命令（验证 Exec URL 流式转发）
#   2. 直接调用 CRI 验证 UpdateContainerResources/CheckpointContainer/ContainerStats/
...
if ! E2B_YAML_COUNT=0 refresh_or_reuse_e2b_yaml "${REFRESH_SCRIPT}" "${POD_NAME}" "${POD_YAML}"; then
    exit 1
fi

if [ ! -f "${POD_YAML}" ]; then
    log_info "Pod YAML 未生成: ${POD_YAML}"
    exit 1
fi

BUILD_ID=$(grep -oP 'e2b\.dev/build-id:\s*"\K[^"]+' "${POD_YAML}" | head -1 || true)
if [ -z "${BUILD_ID}" ]; then
    log_info "无法从 YAML 提取 build_id"
    exit 1
fi
log_info "build_id 已刷新: ${BUILD_ID}"

#==================== 创建 Pod ====================#
log_step "3.1 通过 kubelet 创建 Pod"

if ! kubectl apply -f "${POD_YAML}" >&2 2>&1; then
    log_info "kubectl apply 失败"
    exit 1
fi
log_info "Pod YAML 已提交: ${POD_NAME}"
...
for i in $(seq 1 30); do
    ...
    if [ "${STATUS}" = "Running" ] && [ "${READY_COUNT}" = "true" ]; then
        READY=1
        log_info "Pod 已 Running（第 ${i} 次轮询）"
        break
    fi

    if [ "${STATUS}" = "Failed" ] || [ "${STATUS}" = "Succeeded" ]; then
        log_info "Pod 进入终态: ${STATUS}"
...
if [ "${READY}" -ne 1 ]; then
    log_info "Pod 未在 60s 内进入 Running，当前状态: ${STATUS}"
    kubectl describe pod "${POD_NAME}" >&2 || true
    exit 1
fi
...
if [ -z "${CONTAINER_ID}" ]; then
    ...
    log_info "无法从 kubelet status 读取真实 containerID，回退使用逻辑 ID: ${CONTAINER_ID}"
else
    log_info "读取到真实 containerID: ${CONTAINER_ID}"
fi
...
log_step "5.2.1 观察 CRI Exec 接口调用日志（非验证点）"
...
if [ "${EXEC_COUNT}" -ge 1 ]; then
    log_info "CRI Exec 接口被调用 ${EXEC_COUNT} 次"
else
    log_info "未检测到 CRI Exec 接口调用"
fi
