...
#
# non-cni: 不启用 -cni-enabled，适合 02/04/05/06 crictl 直连用例。
# cni:     启用 -cni-enabled，适合 07/08/09/10/11 kubelet/CNI 用例。
###############################################################################
set -euo pipefail
...
case "${mode}" in
    non-cni|nocni|off|0)
        ...
        CNI_ENABLED=0 "${SCRIPT_DIR}/01_start_multiplex.sh"
        ;;
    cni|on|1)
        log_info "切换 cri-multiplex 到 CNI 模式"
        CNI_ENABLED=1 "${SCRIPT_DIR}/01_start_multiplex.sh"
        ;;
    *)
